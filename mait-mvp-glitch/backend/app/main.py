...
# NOTE: syllabus_service import removed - ChromaDB hangs on import
...
async def startup_event():
    """Startup event - RAG disabled for fast startup."""
    print("MAIT Backend starting...")
    print("NOTE: RAG system DISABLED (ChromaDB hang issue). Using Gemini only.")
...
# Map session_id -> StudentContext
# In a real app, this would be Redis/DB
sessions: Dict[str, StudentContext] = {}
...
def get_or_create_context(student_id: str) -> StudentContext:
    if student_id not in sessions:
        sessions[student_id] = StudentContext(student_id=student_id)
    return sessions[student_id]

@app.get("/context/{student_id}", response_model=StudentContext)
def get_context(student_id: str):
    return get_or_create_context(student_id)

@app.post("/interact")
async def interact(request: InteractionRequest):
    # 1. Retrieve/Create Context
    context = get_or_create_context(request.student_id)
    ...
    if context.fatigue_metric.status == FatigueStatus.LOCKOUT:
        sessions[request.student_id] = context # Save state
        return {
    ...
    if context.fatigue_metric.status == FatigueStatus.LOCKOUT:
         ...
         # Use async Gemini integration - fatigue state is injected into system prompt
         response_text = await educational_agent.generate_response_async(request.query, context)

    # 5. Save Context
    sessions[request.student_id] = context
...
def reset_context(student_id: str):
    sessions[student_id] = StudentContext(student_id=student_id)
    return sessions[student_id]
...
async def submit_keystroke_metrics(submission: KeystrokeSubmission):
    ...
    """
    context = get_or_create_context(submission.student_id)
    ...
    sessions[submission.student_id] = context
...
def get_keystroke_profile(student_id: str):
...
async def subscribe_waitlist(request: SubscribeRequest):
    """Simple JSON storage for waitlist emails"""
    entry = {"email": request.email, "timestamp": str(datetime.now())}
