...
export function RevisionTimeline({ isOpen, onClose }: RevisionTimelineProps) {
  const { revisionHistory, applyRevision, rejectRevision, elementsById } = useCanvasStore();
  ...
  const getElementLabel = (elementId: string) => {
    return elementsById[elementId]?.label || 'Unknown Element';
  };
              ...
              {revisionHistory.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center">
                  ...
                  <p className="text-white/30 text-sm">
                    Select an element and use "Revise with AI" to get started
                  </p>
                </div>
              ) : (
                  ...
                  {revisionHistory.map((revision, index) => (
                    ...
                    >
                      ...
                      <p className="text-white/80 text-sm font-medium mb-1">
                        {getElementLabel(revision.elementId)}
                      </p>
